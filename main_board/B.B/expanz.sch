EESchema Schematic File Version 4
LIBS:B.B-cache
EELAYER 29 0
EELAYER END
$Descr A4 11693 8268
encoding utf-8
Sheet 7 10
Title ""
Date ""
Rev ""
Comp ""
Comment1 ""
Comment2 ""
Comment3 ""
Comment4 ""
$EndDescr
$Comp
L B.B-rescue:PCA9555D-Interface_Expansion U?
U 1 1 5F9EEC47
P 3600 3650
AR Path="/5F9EE529/5F9EEC47" Ref="U?"  Part="1" 
AR Path="/5FA9DBB3/5F9EEC47" Ref="U?"  Part="1" 
F 0 "U?" H 3600 4700 50  0000 C CNN
F 1 "PCA9555D-Interface_Expansion" H 3600 4600 50  0000 C CNN
F 2 "Package_SO:SOIC-24W_7.5x15.4mm_P1.27mm" H 4550 2650 50  0001 C CNN
F 3 "" H 3600 3650 50  0001 C CNN
	1    3600 3650
	1    0    0    -1  
$EndComp
Text HLabel 2500 2850 0    50   Input ~ 0
3V3
Text HLabel 3100 3050 0    50   Input ~ 0
SCL
Text HLabel 3100 3150 0    50   Input ~ 0
SDA
$Sheet
S 5350 2400 550  1300
U 5F9F24D4
F0 "perception00" 50
F1 "perception.sch" 50
F2 "3V3" I L 5350 2450 50 
F3 "SCL" I L 5350 2600 50 
F4 "SDA" I L 5350 2700 50 
F5 "int_accel1" I L 5350 2850 50 
F6 "int_accel2" I L 5350 2950 50 
F7 "int_giro1" I L 5350 3050 50 
F8 "int_giro2" I L 5350 3150 50 
F9 "int_magnet" I L 5350 3250 50 
F10 "INT" I L 5350 3600 50 
F11 "int_RTC" I L 5350 3350 50 
F12 "int_baro" I L 5350 3450 50 
$EndSheet
Text HLabel 5350 2600 0    50   Input ~ 0
SCL
Text HLabel 5350 2700 0    50   Input ~ 0
SDA
Text HLabel 5350 2450 0    50   Input ~ 0
3V3
Wire Wire Line
	5350 2850 4100 2850
Wire Wire Line
	4100 2950 5350 2950
Wire Wire Line
	5350 3050 4100 3050
Wire Wire Line
	4100 3150 5350 3150
Wire Wire Line
	5350 3250 4100 3250
Wire Wire Line
	4100 3350 5350 3350
Wire Wire Line
	5350 3450 4100 3450
Text HLabel 5200 3600 0    50   Input ~ 0
INT
Text HLabel 3100 3350 0    50   Input ~ 0
INT-PCA
Wire Wire Line
	3100 4250 3050 4250
Wire Wire Line
	3050 4250 3050 4350
Wire Wire Line
	3050 4700 3600 4700
Wire Wire Line
	3600 4700 3600 4650
Wire Wire Line
	3100 4450 3050 4450
Connection ~ 3050 4450
Wire Wire Line
	3050 4450 3050 4700
Wire Wire Line
	3100 4350 3050 4350
Connection ~ 3050 4350
Wire Wire Line
	3050 4350 3050 4450
Wire Wire Line
	3600 4700 3600 4750
Connection ~ 3600 4700
$Comp
L B.B-eagle-import:GND #GND?
U 1 1 5FA31D9D
P 3600 4850
AR Path="/5F9EE529/5FA31D9D" Ref="#GND?"  Part="1" 
AR Path="/5FA9DBB3/5FA31D9D" Ref="#GND?"  Part="1" 
F 0 "#GND?" H 3600 4850 50  0001 C CNN
F 1 "GND" H 3600 4770 59  0000 C CNN
F 2 "" H 3600 4850 50  0001 C CNN
F 3 "" H 3600 4850 50  0001 C CNN
	1    3600 4850
	1    0    0    -1  
$EndComp
Text HLabel 4100 4150 2    50   Input ~ 0
IO1_4
Text HLabel 4100 4250 2    50   Input ~ 0
IO1_5
Text HLabel 4100 4350 2    50   Input ~ 0
IO1_6
Text HLabel 4100 4450 2    50   Input ~ 0
IO1_7
$Comp
L Device:C C?
U 1 1 5FA4EEE5
P 2550 3100
AR Path="/5F9EE529/5FA4EEE5" Ref="C?"  Part="1" 
AR Path="/5FA9DBB3/5FA4EEE5" Ref="C?"  Part="1" 
F 0 "C?" H 2665 3146 50  0000 L CNN
F 1 "1uf" H 2665 3055 50  0000 L CNN
F 2 "" H 2588 2950 50  0001 C CNN
F 3 "~" H 2550 3100 50  0001 C CNN
F 4 "C52923" H 2550 3100 50  0001 C CNN "LCSC"
	1    2550 3100
	1    0    0    -1  
$EndComp
Wire Wire Line
	2500 2850 2550 2850
Wire Wire Line
	2550 2950 2550 2850
Connection ~ 2550 2850
Wire Wire Line
	2550 2850 3100 2850
$Comp
L B.B-eagle-import:GND #GND?
U 1 1 5FA4FAE4
P 2550 3350
AR Path="/5F9EE529/5FA4FAE4" Ref="#GND?"  Part="1" 
AR Path="/5FA9DBB3/5FA4FAE4" Ref="#GND?"  Part="1" 
F 0 "#GND?" H 2550 3350 50  0001 C CNN
F 1 "GND" H 2550 3270 59  0000 C CNN
F 2 "" H 2550 3350 50  0001 C CNN
F 3 "" H 2550 3350 50  0001 C CNN
	1    2550 3350
	1    0    0    -1  
$EndComp
$Comp
L Motor:Motor_DC M?
U 1 1 5FA5194A
P 10200 1500
AR Path="/5F9EE529/5FA5194A" Ref="M?"  Part="1" 
AR Path="/5FA9DBB3/5FA5194A" Ref="M?"  Part="1" 
F 0 "M?" H 10358 1496 50  0000 L CNN
F 1 "Motor_DC" H 10358 1405 50  0000 L CNN
F 2 "" H 10200 1410 50  0001 C CNN
F 3 "~" H 10200 1410 50  0001 C CNN
	1    10200 1500
	1    0    0    -1  
$EndComp
$Comp
L Device:Q_NMOS_DSG Q?
U 1 1 5FA59A13
P 9000 1750
AR Path="/5F9EE529/5FA59A13" Ref="Q?"  Part="1" 
AR Path="/5FA9DBB3/5FA59A13" Ref="Q?"  Part="1" 
F 0 "Q?" H 9204 1796 50  0000 L CNN
F 1 "Q_NMOS_DSG" H 9204 1705 50  0000 L CNN
F 2 "" H 9200 1850 50  0001 C CNN
F 3 "~" H 9000 1750 50  0001 C CNN
	1    9000 1750
	1    0    0    -1  
$EndComp
Wire Wire Line
	9300 1250 9900 1250
Wire Wire Line
	10200 1250 10200 1300
Wire Wire Line
	10200 1800 10200 1950
$Comp
L B.B-eagle-import:GND #GND?
U 1 1 5FA5FD95
P 9100 2050
AR Path="/5F9EE529/5FA5FD95" Ref="#GND?"  Part="1" 
AR Path="/5FA9DBB3/5FA5FD95" Ref="#GND?"  Part="1" 
F 0 "#GND?" H 9100 2050 50  0001 C CNN
F 1 "GND" H 9100 1970 59  0000 C CNN
F 2 "" H 9100 2050 50  0001 C CNN
F 3 "" H 9100 2050 50  0001 C CNN
	1    9100 2050
	1    0    0    -1  
$EndComp
$Comp
L B.B-eagle-import:GND #GND?
U 1 1 5FA60281
P 10200 2050
AR Path="/5F9EE529/5FA60281" Ref="#GND?"  Part="1" 
AR Path="/5FA9DBB3/5FA60281" Ref="#GND?"  Part="1" 
F 0 "#GND?" H 10200 2050 50  0001 C CNN
F 1 "GND" H 10200 1970 59  0000 C CNN
F 2 "" H 10200 2050 50  0001 C CNN
F 3 "" H 10200 2050 50  0001 C CNN
	1    10200 2050
	1    0    0    -1  
$EndComp
Text HLabel 8900 1250 0    50   Input ~ 0
5V
Wire Wire Line
	4950 4000 4950 3550
Wire Wire Line
	4950 3550 4100 3550
$Sheet
S 5350 3900 800  600 
U 5F9AAC54
F0 "sheet5F9AAC52" 50
F1 "A9G+.sch" 50
F2 "4V" I R 6150 3950 50 
F3 "3.3V" I R 6150 4050 50 
F4 "PWR_KEY" I L 5350 4250 50 
F5 "RESET" I L 5350 4150 50 
F6 "TXD_A9G" I R 6150 4300 50 
F7 "RXD_A9G" I R 6150 4400 50 
F8 "INT" I L 5350 4000 50 
$EndSheet
Text HLabel 6150 4050 2    50   Input ~ 0
3V3
Text HLabel 6150 3950 2    50   Input ~ 0
4V
Wire Wire Line
	5350 4150 4850 4150
Wire Wire Line
	4850 4150 4850 3750
Wire Wire Line
	4850 3750 4100 3750
Wire Wire Line
	4100 3850 4750 3850
Wire Wire Line
	4750 3850 4750 4250
Wire Wire Line
	4750 4250 5350 4250
Text Label 4100 3950 0    50   ~ 0
WS2812-power
$Comp
L Device:Q_NMOS_DSG Q?
U 1 1 5F9DF808
P 8700 3500
AR Path="/5F9EE529/5F9DF808" Ref="Q?"  Part="1" 
AR Path="/5FA9DBB3/5F9DF808" Ref="Q?"  Part="1" 
F 0 "Q?" H 8904 3546 50  0000 L CNN
F 1 "Q_NMOS_DSG" H 8904 3455 50  0000 L CNN
F 2 "" H 8900 3600 50  0001 C CNN
F 3 "~" H 8700 3500 50  0001 C CNN
	1    8700 3500
	1    0    0    -1  
$EndComp
$Comp
L B.B-eagle-import:GND #GND?
U 1 1 5F9DF814
P 8800 3800
AR Path="/5F9EE529/5F9DF814" Ref="#GND?"  Part="1" 
AR Path="/5FA9DBB3/5F9DF814" Ref="#GND?"  Part="1" 
F 0 "#GND?" H 8800 3800 50  0001 C CNN
F 1 "GND" H 8800 3720 59  0000 C CNN
F 2 "" H 8800 3800 50  0001 C CNN
F 3 "" H 8800 3800 50  0001 C CNN
	1    8800 3800
	1    0    0    -1  
$EndComp
Text HLabel 8600 3000 0    50   Input ~ 0
5V
Text Label 8500 3500 2    50   ~ 0
WS2812-power
Text HLabel 9000 3000 2    50   Input ~ 0
5V-WS2812
Text HLabel 7900 5100 0    50   Input ~ 0
3V3
$Comp
L Device:R R?
U 1 1 5FA465DA
P 8050 5100
AR Path="/5FA3274C/5FA465DA" Ref="R?"  Part="1" 
AR Path="/5F9EE529/5FA465DA" Ref="R?"  Part="1" 
F 0 "R?" V 7843 5100 50  0000 C CNN
F 1 "220" V 7934 5100 50  0000 C CNN
F 2 "" V 7980 5100 50  0001 C CNN
F 3 "~" H 8050 5100 50  0001 C CNN
F 4 "C25092" V 8050 5100 50  0001 C CNN "LCSC"
	1    8050 5100
	0    1    1    0   
$EndComp
$Comp
L B.B-eagle-import:GND #GND?
U 1 1 5FA465E0
P 8800 5550
AR Path="/5FA3274C/5FA465E0" Ref="#GND?"  Part="1" 
AR Path="/5F9EE529/5FA465E0" Ref="#GND?"  Part="1" 
F 0 "#GND?" H 8800 5550 50  0001 C CNN
F 1 "GND" H 8800 5470 59  0000 C CNN
F 2 "" H 8800 5550 50  0001 C CNN
F 3 "" H 8800 5550 50  0001 C CNN
	1    8800 5550
	1    0    0    -1  
$EndComp
Text HLabel 9100 5100 2    50   Input ~ 0
IR-OUT
$Comp
L B.B-eagle-import:LED LED?
U 1 1 5FA465E8
P 9750 5200
AR Path="/5FA3274C/5FA465E8" Ref="LED?"  Part="1" 
AR Path="/5F9EE529/5FA465E8" Ref="LED?"  Part="1" 
F 0 "LED?" H 9828 5203 59  0000 L CNN
F 1 "LED" H 9828 5098 59  0000 L CNN
F 2 "Diode_SMD:D_0402_1005Metric" H 9750 5200 50  0001 C CNN
F 3 "" H 9750 5200 50  0001 C CNN
F 4 "C142316" H 9750 5200 50  0001 C CNN "LCSC"
	1    9750 5200
	1    0    0    -1  
$EndComp
$Comp
L B.B-eagle-import:GND #GND?
U 1 1 5FA465EE
P 9750 5500
AR Path="/5FA3274C/5FA465EE" Ref="#GND?"  Part="1" 
AR Path="/5F9EE529/5FA465EE" Ref="#GND?"  Part="1" 
F 0 "#GND?" H 9750 5500 50  0001 C CNN
F 1 "GND" H 9750 5420 59  0000 C CNN
F 2 "" H 9750 5500 50  0001 C CNN
F 3 "" H 9750 5500 50  0001 C CNN
	1    9750 5500
	1    0    0    -1  
$EndComp
Text HLabel 9750 4800 1    50   Input ~ 0
IR-INPUT
$Comp
L B.B-rescue:IRM-H936 U?
U 1 1 5FA465F6
P 8800 5100
AR Path="/5FA3274C/5FA465F6" Ref="U?"  Part="1" 
AR Path="/5F9EE529/5FA465F6" Ref="U?"  Part="1" 
F 0 "U?" H 8800 5325 50  0000 C CNN
F 1 "IRM-H936" H 8800 5234 50  0000 C CNN
F 2 "" H 8800 5100 50  0001 C CNN
F 3 "https://datasheet.lcsc.com/szlcsc/1912111437_Everlight-Elec-IRM-H936-TR2_C264266.pdf" H 8800 5100 50  0001 C CNN
F 4 "C264266" H 8800 5100 50  0001 C CNN "LCSC"
	1    8800 5100
	1    0    0    -1  
$EndComp
$Comp
L Device:D D?
U 1 1 5F99F1C9
P 5250 3800
AR Path="/5F96CFDA/5F99F1C9" Ref="D?"  Part="1" 
AR Path="/5F9EE529/5F9F24D4/5F99F1C9" Ref="D?"  Part="1" 
AR Path="/5FA9DBB3/5F9F24D4/5F99F1C9" Ref="D?"  Part="1" 
AR Path="/5F9EE529/5F99F1C9" Ref="D?"  Part="1" 
F 0 "D?" H 5450 3750 50  0000 R CNN
F 1 "D" V 5150 3800 50  0000 R CNN
F 2 "BB:SOD-323" H 5250 3800 50  0001 C CNN
F 3 "https://datasheet.lcsc.com/szlcsc/SK-SD103AWS_C109204.pdf" H 5250 3800 50  0001 C CNN
F 4 "C109204" V 5250 3800 50  0001 C CNN "LCSC"
	1    5250 3800
	0    1    1    0   
$EndComp
Wire Wire Line
	5200 3600 5250 3600
Wire Wire Line
	4950 4000 5250 4000
Wire Wire Line
	5250 3950 5250 4000
Connection ~ 5250 4000
Wire Wire Line
	5250 4000 5350 4000
Wire Wire Line
	5250 3650 5250 3600
Connection ~ 5250 3600
Wire Wire Line
	5250 3600 5350 3600
Text HLabel 6150 4300 2    50   Input ~ 0
TXD_A9G
Text HLabel 6150 4400 2    50   Input ~ 0
RXD_A9G
$Sheet
S 10000 3000 500  500 
U 5FA93863
F0 "enc00" 50
F1 "enc.sch" 50
F2 "3V3" I L 10000 3050 50 
F3 "OUT_1" I L 10000 3400 50 
F4 "OUT_0" I L 10000 3300 50 
$EndSheet
Text HLabel 10000 3050 0    50   Input ~ 0
3V3
Text HLabel 10000 3300 0    50   Input ~ 0
ENC_OUT_0
Text HLabel 10000 3400 0    50   Input ~ 0
ENC_OUT_1
$Comp
L Device:Q_PMOS_DGS Q?
U 1 1 5FAED176
P 9100 1350
F 0 "Q?" V 9443 1350 50  0000 C CNN
F 1 "Q_PMOS_DGS" V 9352 1350 50  0000 C CNN
F 2 "" H 9300 1450 50  0001 C CNN
F 3 "~" H 9100 1350 50  0001 C CNN
	1    9100 1350
	0    1    -1   0   
$EndComp
$Comp
L Device:Q_PMOS_DGS Q?
U 1 1 5FAEED74
P 8800 3100
F 0 "Q?" V 9143 3100 50  0000 C CNN
F 1 "Q_PMOS_DGS" V 9052 3100 50  0000 C CNN
F 2 "" H 9000 3200 50  0001 C CNN
F 3 "~" H 8800 3100 50  0001 C CNN
	1    8800 3100
	0    1    -1   0   
$EndComp
$Comp
L Device:D_Schottky D?
U 1 1 5FAF23FC
P 9900 1600
F 0 "D?" V 9850 1450 50  0000 L CNN
F 1 "D_Schottky" V 9950 1100 50  0000 L CNN
F 2 "" H 9900 1600 50  0001 C CNN
F 3 "~" H 9900 1600 50  0001 C CNN
	1    9900 1600
	0    1    1    0   
$EndComp
Wire Wire Line
	9900 1450 9900 1250
Connection ~ 9900 1250
Wire Wire Line
	9900 1250 10200 1250
$Comp
L B.B-eagle-import:GND #GND?
U 1 1 5FAF4CA6
P 9900 2050
AR Path="/5F9EE529/5FAF4CA6" Ref="#GND?"  Part="1" 
AR Path="/5FA9DBB3/5FAF4CA6" Ref="#GND?"  Part="1" 
F 0 "#GND?" H 9900 2050 50  0001 C CNN
F 1 "GND" H 9900 1970 59  0000 C CNN
F 2 "" H 9900 2050 50  0001 C CNN
F 3 "" H 9900 2050 50  0001 C CNN
	1    9900 2050
	1    0    0    -1  
$EndComp
Wire Wire Line
	9900 1950 9900 1750
Text HLabel 4100 4050 2    50   Input ~ 0
IO1_3
Text HLabel 8800 1750 0    50   Input ~ 0
MOTOR
$Comp
L Device:C C?
U 1 1 5FB0693F
P 8250 5300
AR Path="/5FA4C94D/5FB0693F" Ref="C?"  Part="1" 
AR Path="/5F9AAC80/5FB0693F" Ref="C?"  Part="1" 
AR Path="/5F9EE529/5FB0693F" Ref="C?"  Part="1" 
F 0 "C?" H 8365 5346 50  0000 L CNN
F 1 "2.2uf" H 8365 5255 50  0000 L CNN
F 2 "Capacitor_SMD:C_0805_2012Metric" H 8288 5150 50  0001 C CNN
F 3 "~" H 8250 5300 50  0001 C CNN
F 4 "C49217" H 8250 5300 50  0001 C CNN "LCSC"
	1    8250 5300
	1    0    0    -1  
$EndComp
$Comp
L B.B-eagle-import:GND #GND?
U 1 1 5FB06945
P 8250 5550
AR Path="/5FA4C94D/5FB06945" Ref="#GND?"  Part="1" 
AR Path="/5F9AAC80/5FB06945" Ref="#GND?"  Part="1" 
AR Path="/5F9EE529/5FB06945" Ref="#GND?"  Part="1" 
F 0 "#GND?" H 8250 5550 50  0001 C CNN
F 1 "GND" H 8250 5470 59  0000 C CNN
F 2 "" H 8250 5550 50  0001 C CNN
F 3 "" H 8250 5550 50  0001 C CNN
	1    8250 5550
	1    0    0    -1  
$EndComp
Wire Wire Line
	8250 5100 8250 5150
Wire Wire Line
	8200 5100 8250 5100
Connection ~ 8250 5100
Wire Wire Line
	8250 5100 8500 5100
$Comp
L Device:R R?
U 1 1 5FA5476F
P 9750 4950
AR Path="/5FA3274C/5FA5476F" Ref="R?"  Part="1" 
AR Path="/5F9EE529/5FA5476F" Ref="R?"  Part="1" 
F 0 "R?" V 9543 4950 50  0000 C CNN
F 1 "220" V 9634 4950 50  0000 C CNN
F 2 "" V 9680 4950 50  0001 C CNN
F 3 "~" H 9750 4950 50  0001 C CNN
F 4 "C25092" V 9750 4950 50  0001 C CNN "LCSC"
	1    9750 4950
	-1   0    0    1   
$EndComp
$EndSCHEMATC
